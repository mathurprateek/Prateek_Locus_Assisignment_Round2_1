...
for i in range(1, 4):
    tm = unix_timestamp - (86400*i)  # evaluating timestamp for last three days
    res = requests.get('https://api.openweathermap.org/data/2.5/onecall/timemachine?lat={}&lon={}&dt={}'
                       '&appid=78385f931961ae9b80060c70d2c7bf6f'.format(lat, lon, tm))  # API calling
    output_dict[str(res.json()['current']['dt']) + ' ' + 'Unix Time'] \
        = str(res.json()['current']['pressure']) + ' ' + 'Millibar'  # store response in dictionary
